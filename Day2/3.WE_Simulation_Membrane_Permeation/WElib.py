...
class StaticBinner(object):
    ...
    def run(self, walkers):
        if not isinstance(walkers, list):
            walkers = [walkers]
        pcs = np.atleast_2d([w.pc for w in walkers])
        if None in pcs:
...
class Recycler(object):
    ...
    def __init__(self, initial_state, target_pc, retrograde=False):
        ...
        if isinstance(target_pc, (list, tuple)):
            self.n_dim = len(target_pc)
        else:
            self.n_dim = 1
        if self.n_dim == 1:
            self.retrograde = retrograde
        ...
        for i in range(len(walkers)):
            ...
            if self.n_dim == 1:
                ...
                for idim in range(self.n_dim):
                    if self.target_pc[idim] is not None:
                        if not self.retrograde[idim]:
                            recycle = recycle or walkers[i].pc[idim] > self.target_pc[idim]
                        else:
                            recycle = recycle or walkers[i].pc[idim] < self.target_pc[idim]
            if recycle:
